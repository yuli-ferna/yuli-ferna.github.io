          ...
          {localesAndNames.map(({ locale, name }) => {
            ...
            return (
              <div
                ...
                className={`relative w-auto cursor-pointer select-none px-4 py-2 text-black hover:bg-zinc-200`}
              >
                <span className={`block truncate ${isSelected && 'font-bold text-primary'}`}>{name}</span>
              </div>
            )
          })}
          <PopoverArrow />
